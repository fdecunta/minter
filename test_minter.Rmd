...
my_eff <- inter_effsize(effsize = "lnrr",
                        c("Env", "Str"),
                        CC_mean, CC_SD, CC_n ,
                        EC_mean, EC_SD, EC_n, 
                        CS_mean, CS_SD, CS_n, 
...
```{r}
# Environment lnRR
round(my_eff$Env_overall_lnRR - effect_size$lnRR_E, 7)
```


```{r}
# Environment lnRR var
round(my_eff$Env_overall_lnRR_var - effect_size$lnRRV_E, 7)
```

Test overall effect stress

```{r}
# Stress lnRR
round(my_eff$Str_overall_lnRR - effect_size$lnRR_S, 7)
```


```{r}
# Stress lnRR var
round(my_eff$Str_overall_lnRR_var - effect_size$lnRRV_S, 7)
```
...
```{r}
round(my_eff$Env_x_Str_lnRR - effect_size$lnRR_ES, 7)
```

```{r}
round(my_eff$Env_x_Str_lnRR_var - effect_size$lnRRV_ES, 7)
```
