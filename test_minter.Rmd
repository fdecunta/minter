...
```{r}
round(my_eff$Env_overall_lnRR - effect_size$lnRR_E, 4)
```

Check the same but for the var

```{r}
my_eff$Env_overall_lnRR_var - effect_size$lnRRV_E
```


### Test overall effect environment?

```{r}
round(my_eff$Str_overall_lnRR - effect_size$lnRR_S, 4)
```
...
```{r}
round(my_eff$inter_lnRR - effect_size$lnRR_ES, 4)
```
